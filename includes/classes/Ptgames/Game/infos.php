...
function BuildFleetListRows ( $CurrentPlanet ) {
	...
	$RowsTPL  = gettemplate('Game/gate_fleet_rows');
	$CurrIdx  = 1;
	$Result   = "";
...
function ShowBuildingInfoPage ($CurrentUser, $CurrentPlanet, $BuildID) {
	...
	if       ($BuildID >=   1 && $BuildID <=   3) {
		// Cas des mines
		$PageTPL              = gettemplate('Game/info_buildings_table');
		$DestroyTPL           = gettemplate('Game/info_buildings_destroy');
		...
		// Batiments Generaux
		$PageTPL              = gettemplate('Game/info_buildings_general');
		$DestroyTPL           = gettemplate('Game/info_buildings_destroy');
	} elseif ($BuildID ==  33) {
		...
		// Porte de Saut
		$PageTPL              = gettemplate('Game/info_buildings_general');
		$GateTPL              = gettemplate('Game/gate_fleet_table');
		$DestroyTPL           = gettemplate('Game/info_buildings_destroy');
		...
		// Flotte
		$PageTPL              = gettemplate('Game/info_buildings_fleet');
		...
		// Defenses
		$PageTPL              = gettemplate('Game/info_buildings_defense');
		...
		// Officiers
		$PageTPL              = gettemplate('Game/info_officiers_general');
	}
	...
	if ($DestroyTPL != '') {
		if ($CurrentPlanet[$resource[$BuildID]] > 0) {
			...
			$parse['destroyurl']  = "game.php?page=BatimentBuildingPage&cmd=destroy&building=".$BuildID; // Non balisé les balises sont dans le tpl
